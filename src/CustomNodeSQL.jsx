...
function CustomNodeSQL({ data: nodeData, targetPosition, sourcePosition }) {
  // Verificar se o ID do nó é '9'
  const isResultNode = nodeData.id === '11';
  ...
  useEffect(() => {
    const intervalId = setInterval(() => {
      axios.get('http://localhost:5000/sql')
        .then(response => {
...
export default memo(CustomNodeSQL);
