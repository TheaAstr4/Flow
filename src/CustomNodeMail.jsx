...
function CustomNodeMail({ data: nodeData, targetPosition, sourcePosition }) {
  // Verificar se o ID do nó é '9'
  const isResultNode = nodeData.id === '12';
  ...
  useEffect(() => {
    const intervalId = setInterval(() => {
      axios.get('http://localhost:5000/mail')
        .then(response => {
        ...
        <div className="ml-2">
          {apiData.length > 0 ? (
            <>
              ...
              <div className="text-md">{apiData[0]}</div>
            </>
          ) : (
