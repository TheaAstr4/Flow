...
function CustomNodeAgent1({ data: nodeData, targetPosition, sourcePosition }) {
  // Verificar se o ID do nó é '9'
  const isResultNode = nodeData.id === '10';
  ...
  useEffect(() => {
    const intervalId = setInterval(() => {
      axios.get('http://localhost:5000/agent1')
        .then(response => {
...
export default memo(CustomNodeAgent1);
